...
# RUN if [ ! -d "${HADOOP_HOME}" ]; then
    ...
    RUN tar -xvf hadoop-${HADOOP_VERSION}.tar.gz -C /usr/local
    RUN rm hadoop-${HADOOP_VERSION}.tar.gz
    RUN mv /usr/local/hadoop-${HADOOP_VERSION} ${HADOOP_HOME}
    RUN wget -q https://jdbc.postgresql.org/download/postgresql-42.2.5.jar -O ${HADOOP_HOME}/share/hadoop/common/lib/postgresql-42.2.5.jar
    # fi
...
# RUN if [ ! -d "${SQOOP_HOME}" ]; then
    ...
    RUN tar -xvf sqoop-${SQOOP_VERSION}.bin__hadoop-2.6.0.tar.gz -C /usr/local
    RUN rm sqoop-${SQOOP_VERSION}.bin__hadoop-2.6.0.tar.gz
    RUN mv /usr/local/sqoop-${SQOOP_VERSION}.bin__hadoop-2.6.0 ${SQOOP_HOME}
    RUN cp ${HADOOP_HOME}/share/hadoop/common/lib/postgresql-42.2.5.jar ${SQOOP_HOME}/lib/
    RUN rm -f ${SQOOP_HOME}/lib/commons-lang3-*.jar
    RUN cp ${HADOOP_HOME}/share/hadoop/common/lib/commons-lang-2.6.jar ${SQOOP_HOME}/lib/
    # fi
